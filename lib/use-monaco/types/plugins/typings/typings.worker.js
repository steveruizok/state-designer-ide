...
const store = new idb_keyval_1.Store('typescript-definitions-cache-v1');
...
const fetchFromDefinitelyTyped = (dependency, version, fetchedPaths) => doFetch(`${ROOT_URL}npm/@types/${dependency
    .replace('@', '')
    .replace(/\//g, '__')}/index.d.ts`).then((typings) => {
    fetchedPaths[`node_modules/${dependency}/index.d.ts`] = typings;
});
const getRequireStatements = (title, code) => {
    const requires = [];
    const sourceFile = self.ts.createSourceFile(title, code, self.ts.ScriptTarget.Latest, true, self.ts.ScriptKind.TS);
...
const transformFiles = (dir) => dir.files
    ...
    : {};
const getFileMetaData = (dependency, version, depPath) => doFetch(`https://data.jsdelivr.com/v1/package/npm/${dependency}@${version}/flat`)
...
const getFileTypes = (depUrl, dependency, depPath, fetchedPaths, fileMetaData) => {
    const virtualPath = path_browserify_1.default.join('node_modules', dependency, depPath);
    ...
    return doFetch(`${depUrl}/${depPath}`).then((typings) => {
        ...
        (dep) => dep.startsWith('.'))
            .map((relativePath) => path_browserify_1.default.join(path_browserify_1.default.dirname(depPath), relativePath))
            .map((relativePath) => resolveAppropiateFile(fileMetaData, relativePath))
            .map((nextDepPath) => getFileTypes(depUrl, dependency, nextDepPath, fetchedPaths, fileMetaData)));
    });
};
function fetchFromMeta(dependency, version, fetchedPaths) {
    ...
    return doFetch(depUrl)
        .then((response) => JSON.parse(response))
        .then((meta) => {
        const filterAndFlatten = (files, filter) => files.reduce((paths, file) => {
...
function fetchFromTypings(dependency, version, fetchedPaths) {
    ...
    return doFetch(`${depUrl}/package.json`)
        ...
        if (types) {
            ...
            return getFileMetaData(dependency, version, path_browserify_1.default.join('/', path_browserify_1.default.dirname(types))).then((fileData) => getFileTypes(depUrl, dependency, resolveAppropiateFile(fileData, types), fetchedPaths, fileData));
        }
    ...
    return idb_keyval_1.get(key, store)
        ...
        fetchFromDefinitelyTyped(name, version, fetchedPaths))
            ...
            if (Object.keys(fetchedPaths).length) {
                ...
                idb_keyval_1.set(key, fetchedPaths, store);
                return fetchedPaths;
            }
            else {
...
class TypingsWorker extends worker_1.MonacoWorker {
...
exports.TypingsWorker = TypingsWorker;
...
//   );
// });
//# sourceMappingURL=typings.worker.js.map